...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  // Случай сходных признаков - дополнение
...
int AddAlgo(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int chng_pos = 0;  // Позволяет изменить положение
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 32; j++) {
      ...
      temp_res ^= chng_pos;  // изменить положение, если это необходимо
      if (!chng_pos) {       // случаи, когда требуется смена позиции
