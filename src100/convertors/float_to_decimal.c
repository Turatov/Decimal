...
int s21_from_float_to_decimal(float src, s21_decimal *dst) {
  ...
  if (src < 0) {
    src = -src;
    // dst->bits[3] = 1
    dst->bits[3] = 1 << 31;  // проверка на знак
  }
  ...
  if (src < DEC_MAX) {
    ...
    if (src) {
      int count = 1;
      while (count != 7 && float_tmp < 1000000) {  // 1 000 000
    ...
    // 10 000 000
    // 1234567.25
    while (float_tmp > 10000000) {  // 10 000 000
      float_tmp /= 10;
      exp++;
    }
    ...
    // проверка в какой разряд и запись
